...
为管理员提供 5 个数据统计 API，全部查询今日（`LocalDate.now()`）数据，无需额外参数。按 Clean Architecture 模式拆分为 5 个独立 UseCase，各自继承 `AdminAuthUseCase`，遵循 Template Method + Presenter 模式。
...
`WebApiStatsPresenter` 继承 `WebApiPresenter`，实现 5 个 Presenter 接口，使用 `ResponseContext`（ThreadLocal）写入 HTTP 响应。`SystemTaskBinder` 将其以 Singleton 多绑到 5 个接口上。
